...
currentEuler = eulersRL(i0, r, l, timeSteps);
currentEulerPC = eulersRLPC(i0, r, l, timeSteps);
currentExact = i0.*exp(-timeSteps./tau);
...
function [approx] = eulersRL(i0, r, l, timeSteps)
...
function [approx] = eulersRLPC(i0, r, l, timeSteps)
    nSteps = numel(timeSteps); % num of time steps
    h = timeSteps(2)-timeSteps(1); % time step size
    ...
    for step = 1:nSteps-1 % for every step, perform an iteration
        ...
        yPC(step+1) = yPC(step)+h*((-r/l)*(yPC(step)+yEst)/2); % perform predictor-corrector approx
    end
    approx = yPC;
end